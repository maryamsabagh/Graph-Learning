...
from torch_geometric.nn.conv import LGConv, GATConv, SAGEConv, ChebConv
...
class GCN(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # number of hops in the ChebConv filter.  
        self.kval = 3
        ...
        elif conv_layer == "CHEB":
          #  initialize GraphSAGE conv
          self.convs = ModuleList(
              [ChebConv(in_channels = embedding_dim, out_channels = embedding_dim, K=self.kval, **kwargs) for _ in range(num_layers)]
          )
    ...
    def recommendation_loss(self, pos_edge_rank: Tensor, neg_edge_rank: Tensor, num_neg_edges: int = 1, 
                            lambda_reg: float = 1e-4, **kwargs) -> Tensor:
        r"""Computes the model loss for a ranking objective via the Bayesian
        Personalized Ranking (BPR) loss."""
        loss_fn = BPRLoss(lambda_reg, **kwargs)
        return loss_fn(pos_edge_rank, neg_edge_rank, num_neg_edges, self.embedding.weight)
