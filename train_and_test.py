import torch
from sampling_methods import sample_negative_edges, sample_hard_negative_edges, sample_negative_edges_fast
...
# Train
def train(datasets, model, optimizer, loss_fn, args, n_tracks, n_playlists, num_neg_edges, track_artist_edge_index, 
          neg_samp = "random"):
  print(f"Beginning training for {model.name} w artist nodes")
  ...
  for epoch in range(args["epochs"]): # loop over each epoch
    ...
    if neg_samp == "random":
      neg_edge_index, neg_edge_label = sample_negative_edges(train_data, n_playlists, n_tracks, num_neg_edges, args["device"])            
    elif neg_samp == "hard":
      if epoch % 5 == 0:
    ...
    # calculate embedding w/o artists
    embed = model.get_embedding(train_data.edge_index)
    
    # calculate embedding w artists
    ...
    my_regularization_choice = 1e-8

    # calculate loss function
    if loss_fn == "BCE":
      loss = model.link_pred_loss(scores, labels)
    elif loss_fn == "BPR":
      loss = model.recommendation_loss(pos_scores, neg_scores, num_neg_edges, lambda_reg = my_regularization_choice)
...
#test
    val_loss, val_roc, val_neg_edge, val_neg_label = test(
        model, val_data, loss_fn, neg_samp, args, n_tracks, n_playlists, num_neg_edges, epoch, val_neg_edge, val_neg_label, track_artist_edge_index
    )
    ...
    if epoch % 10 == 0:
      # calculate recall @ K
      val_recall = recall_at_k(val_data, model, k = 392, device = args["device"])
...
def test(model, data, loss_fn, neg_samp, args, n_tracks, n_playlists, num_neg_edges, epoch = 0, neg_edge_index = None, neg_edge_label = None , track_artist_edge_index=None
        ):
  ...
  with torch.no_grad(): # want to save RAM
    ...
    if neg_samp == "random":
      neg_edge_index, neg_edge_label = sample_negative_edges(data, n_playlists, n_tracks, num_neg_edges, args["device"])      
    elif neg_samp == "hard":
    ...
    # calculate embedding w artists
    #full_edge_indices = torch.cat((data.edge_index, track_artist_edge_index), dim=1)
    #embed = model.get_embedding(full_edge_indices)
    ...
    if loss_fn == "BCE":
      ...
      loss = model.recommendation_loss(pos_scores, neg_scores, num_neg_edges, lambda_reg = 0)
