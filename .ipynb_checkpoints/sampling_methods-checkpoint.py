...
def sample_negative_edges_fast(data, num_playlists, num_tracks, num_neg_edges, device=None):
    ...
    sampled_negative_indices = negative_indices[
        torch.randint(0, negative_indices.size(0), size=(positive_playlists.size(0)*num_neg_edges,), device=device)
    ]
...
def sample_negative_edges(data, num_playlists, num_tracks, num_neg_edges, device=None):
    positive_playlists, positive_tracks = data.edge_label_index
    unique_pos_playlist_ids = torch.unique(positive_playlists, sorted=True)        
    
    num_pos_playlists = unique_pos_playlist_ids.size(0)
    num_pos_tracks = positive_tracks.size(0)
    
    negative_tracks = torch.zeros(num_pos_tracks, num_neg_edges, device=device, dtype=torch.long)
    track_ids = torch.arange(num_tracks, device=device, dtype=torch.long)            
    mask = torch.zeros(num_tracks, device=device, dtype=torch.bool)
    
    for jj in range(num_pos_playlists):
        pos_inds = positive_playlists == unique_pos_playlist_ids[jj]
        mask[positive_tracks[pos_inds]-num_playlists] = True
        neg_track_ids = track_ids[mask == False]
        mask[:] = False
        shuffled_ids = neg_track_ids[torch.randperm(int(neg_track_ids.size(0)), device=device)] 
        negative_tracks[pos_inds,:] = shuffled_ids[:num_neg_edges]
        
    negative_tracks += num_playlists                            
    neg_edge_index = torch.stack((positive_playlists.repeat_interleave(num_neg_edges), negative_tracks.flatten()), dim=0)
    neg_edge_label = torch.zeros(neg_edge_index.shape[1], device=device)
