...
def recall_at_k(data, model, k = 392, batch_size = 64, n_playlists = 1000, device = None):
    with torch.no_grad():
    ...
    if recall_at_k.numel() == 1:
        print(f"Total hits: {hits_tensor.sum().item()}")
        print(f"Total relevant: {relevant_counts_tensor.sum().item()}")
        print(f"Mean Recall@{k}: {recall_at_k.item():.4f}")
