...
os.chdir(MAIN_DIR)
AMAZON_DATA_DIR = MAIN_DIR + '/amazon_example/amazon_data'

kcore=26
amazon_data = ld.Data(AMAZON_DATA_DIR,kcore)
Gred = amazon_data.G
...
pickle.dump(Gred, open(f"amazon_{kcore}core.pkl", "wb"))
...
model = GCN(
    ...
    embedding_dim = args["emb_size"], conv_layer = "CHEB"
)
...
MODEL_STATS_DIR = MAIN_DIR+"/amazon_analysis/a_model_stats"
...
for run in range(runs):
    ...
    stats = train(datasets, model, optimizer, "BPR", args, num_items, num_users, num_neg_edges, #track_artist_edge_index,
                  neg_samp = "random")
    pickle.dump(stats, open(f"{MODEL_STATS_DIR}/{num_neg_edges}_neg/{model.name}_run{run}_{num_neg_edges}_1e-8_BPR_random_.pkl", "wb"))
